...
void UInventory::LoadWeapon(USkeletalMeshComponent* Mesh)
{
	...
	{
		if (AWeapon* Weapon = GetWorld()->SpawnActor<AWeapon>(WeaponType, FVector::ZeroVector, FRotator::ZeroRotator,
		                                                      SpawnParams))
		{
	...
	GetCurrentWeapon()->AttachToComponent(Mesh, FAttachmentTransformRules::SnapToTargetNotIncludingScale,
	                                      TEXT("Weapon_R"));
}
...
void UInventory::PickWeapon(AWeapon* NewWeapon, AGT3_Project5_Gr1Character* NewPlayer)
{
	...
	NewWeapon->AttachToComponent(NewPlayer->GetMesh(), FAttachmentTransformRules::SnapToTargetNotIncludingScale,
	                             TEXT("Weapon_R"));
